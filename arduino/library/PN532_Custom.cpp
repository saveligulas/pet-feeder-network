...
void PN532_Custom::begin() {
    ...
    SPI.setClockDivider(SPI_CLOCK_DIV16); // 1 MHz on 16MHz arduino
...
void PN532_Custom::SAMConfig() {
    // SAM Config: Mode, Timeout (0x14 * 50ms = 1s), IRQ
    uint8_t cmd[] = {
...
uint8_t PN532_Custom::readPassiveTargetID(uint8_t cardType, uint8_t* uid, uint8_t* uidLength) {
    // InListPassiveTarget: MaxTg (one card at a time), BrTy (0x00 ISO14443A)
    uint8_t cmd[] = {
    ...
    // Tag check: D5 4B (NbTg, Tg, SENS_RES, SEL_RES, NFCID Length, NFCID)
...
void PN532_Custom::sendCommandCheckAck(const uint8_t* cmd, uint8_t cmdlen, uint16_t timeout) {
    ...
    // skip ack check for now (not important)
}